...
from .forms import AddBUC, AddTar, AddUser, AddPaid, AddRec, AddTC, AddCl
...
def index_paid_receipt(request):
    form_pr = AddPaid()
    data = paid_receipt.objects.all()
    return render(request, "LDCCompany/Template_Paid.html", {"form":form_pr, "data_show":data})
...
class view_paid_receipt(View):
    def add_paid_receipt(request):
        if request.method == "POST":
            paid = paid_receipt()
            paid.id_receipt = request.POST.get("id_receipt")
            paid.payment = request.POST.get("payment")
            paid.save()
            return HttpResponseRedirect("/paid")

    def del_paid_receipt(request):
        if request.method == "POST":
            q = request.POST.get("delname", "")
            que = paid_receipt.objects.get(id=q)
            que.delete()
            return HttpResponseRedirect("/paid")

    def update_paid_receipt(request):
        if request.method == "POST":
            q = request.POST.get("upname", "")
            que = paid_receipt.objects.get(id=q)
            que.id_receipt = request.POST.get("id_receipt")
            que.payment = request.POST.get("payment")
            que.save()
            return HttpResponseRedirect("/paid")
...
class view_receipt(View):
    ...
    def update_receipt(request):
        if request.method == "POST":
            ...
            que.id_paid = paid_receipt.objects.get(id=request.POST.get("id_paid"))
...
class view_conversation(View):
    def add_conversation(request):
        if request.method == "POST":
            ...
            conversation.id_conversation = request.POST.get("id_conversation")
            conversation.save()
    ...
    def update_conversation(request):
        if request.method == "POST":
            ...
            que.id_conversation = request.POST.get("id_conversation")
            que.save()
            return HttpResponseRedirect("/conversation")
